...
For the purpose of this tutorial we will subset samples measured in batches 7 
and 8.

```{r}
MTBLS79 <- MTBLS79[, MTBLS79$Batch %in% c(1:6)]
MTBLS79
```

This data set before peak matrix filtering contains 22 samples, 2488 features
and 11627 missing values across all samples.
...
```{r plot_glog}
glog_plot_optimised_lambda(df=MTBLS79_mv_imputed, optimised_lambda=97102173,
    classes=MTBLS79_filtered$Class, qc_label="QC")
```
...
`r Biocpkg("SummarizedExperiment")` class objet. For example, if input is `matrix`
with features stored in columns and sample in rows, any function of `pmp` 
package will be able to handle this object.
